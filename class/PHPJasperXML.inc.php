    ...
    $this->cndriver=$cndriver;
        if($cndriver=="mysql") {
            if(!$this->con) {
                ...
                if($this->myconn) {
                    $seldb = @mysqli_select_db($myconn,$db_or_dsn_name);
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
        $this->m=0;
        $mysqli = new mysqli($host,$user,$password,$db_or_dsn_name);
        ...
        if($cndriver=="odbc") {
            ...
            // $roww = $result->fetch_array(MYSQLI_ASSOC);
// print_r($roww);
// exit;
...
//echo "$pchartfolder/class/pData.class.php";die;
    ...
    if($ds!=""){
        ...
        $result = $mysqli->query($sql); //query from db
        $roww = $result->fetch_array(MYSQLI_ASSOC);
...
/*       $rowheight="";
   ...
   $this->pdf->writeHTML($table);
    ...
    public function query($sql){
        $mysqlii = new mysqli($host,$user,$password,$db_or_dsn_name);

        return @mysqli_query($mysqlii, $sql);
    }
